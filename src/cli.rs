...
{   /// Display usage information
    ...
    {   // display actual menu
        ...
        let chosen_file =
            get_prompt_input( 
                &format!( "Drag a BSP/ENT file or folder you want to {choice:?} (leave blank to use the current folder, enter 'x' to cancel)\n" ) );
        // User quits
        if chosen_file == "x"
        {
            return Ok( true );
        }
        // Open the BSP/ENT file in the editor        
        if matches!( choice, Self::Edit ) 
        {
            editor::launch( chosen_file )?;
            return Ok( true );
        }

        let chosen_bsp =// Passed in a ENT file, swap extension, want BSPs only
        if let path = PathBuf::from( &chosen_file ) 
...
/// Gets a single keystroke from the user, returns the key that was pressed.
/// Blocks until a key is pressed
pub(crate) fn get_keystroke() -> Result<char>
{
    enable_raw_mode()?;
    loop
    {
...
/// Gets input from the user with a prompt, returns the input as a String
/// Blocks.
pub(crate) fn get_prompt_input(prompt: &str) -> String
